...
Everything about *how this project is run* lives here. Code lives in `src/`,
tests in `tests/`, frozen per-feature spec-kit artifacts in `specs/NNN-*/`.
Everything else — why the project exists, what we're investigating, what we've
designed, what we're building, and what happened — lives in one of the five
areas below.
...
| [`02-DESIGN/`](02-DESIGN/README.md) | Architecture & feature designs, normative specs, evidence trails | When research graduates, or a build changes behavior |
...
```
01-RESEARCH ──graduates──▶ 02-DESIGN ──/speckit-specify──▶ specs/NNN + code
     │                         ▲                                │
...
- Designs in `02-DESIGN/` are written functional-level and explicit enough to
  hand to `/speckit-specify`. Implementation always goes through the spec-kit
  flow; research never does.
- The lifecycle transitions are mechanized as skills: `/research-start`,
  ...
  `tests/test_hq_structure.py`, which rides the standard quality gate.
