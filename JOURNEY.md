...
measurement (Chapter 12): the landing level is set by a **flat score basin**
(span ≈ 0.02 across dims 6–16), not by any age asymmetry. Open research, in
priority order: **the scorer question** (Doc 03 §6 — make the basin
informative: re-shaped parsimony / elbow-relative selection / frozen
shared-window judge; design-level, needs reference revalidation), T3's
persistence clause at scale, the curiosity/competence blend for
