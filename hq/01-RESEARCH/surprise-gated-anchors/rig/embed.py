...
class Anchors:
    ...
    def observe(self, a: int, err_mag: float) -> None:
        pass

    def mobile(self, a: int) -> bool:
        return True


class SurpriseGatedAnchors(Anchors):
    """Mobility priced by evidence (topic surprise-gated-anchors,
    design 0019 constraint 2): a per-act prediction-error EMA E_a
    beside a global EMA G; the anchor row moves only while
    E_a > theta * G. Lawful acts converge below the gate and freeze
    themselves; exceptional acts stay above it and keep mobility."""

    def __init__(self, table, lr: float = 0.01, alpha: float = 0.1, theta: float = 2.0):
        super().__init__(table, learn=True, lr=lr)
        self.alpha = float(alpha)
        self.theta = float(theta)
        self.Ea = np.full(self.table.shape[0], np.nan)
        self.G = float("nan")
        self.moves = 0
        self.observations = 0
    ...
    def observe(self, a: int, err_mag: float) -> None:
        ...
        self.Ea[a] = (
            err_mag
            if np.isnan(self.Ea[a])
            else (1 - self.alpha) * self.Ea[a] + self.alpha * err_mag
        )
        self.G = err_mag if np.isnan(self.G) else (1 - self.alpha) * self.G + self.alpha * err_mag

    def mobile(self, a: int) -> bool:
        if np.isnan(self.Ea[a]) or np.isnan(self.G):
            return False
        m = bool(self.Ea[a] > self.theta * self.G)
        if m:
            self.moves += 1
        return m
...
class EmbeddedFrameGroup(FrameGroup):
    ...
    def learn_transition(
        ...
        err = pred - target
        if m.sum() > 0:
            self.anchors.observe(a, float(np.abs(err[elect]).mean()))
        ...
        if self.anchors.learn and self.anchors.mobile(a):
...
def run_arm(
    ...
    if learn_anchors == "gated":
        anchors = SurpriseGatedAnchors(factored_anchors_v2(m))
    else:
        lr = 0.001 if edition == 3 else 0.01
        anchors = Anchors(table, learn=bool(learn_anchors), lr=lr)
    ...
    row["arm"] = "gated" if learn_anchors == "gated" else ("learned" if learn_anchors else "frozen")
    if isinstance(anchors, SurpriseGatedAnchors):
        row["gate_moves"] = anchors.moves
        row["gate_obs"] = anchors.observations
    return row
...
def arm_sweep(arm: str, m: int, n_cycles: int, edition: int = 2) -> None:
    ...
    tag = arm if (edition == 1 or arm == "gated") else f"{arm}-v{edition}"
    ...
    with out.open("a") as f:
        for seed in range(dial.SEEDS):
            la = "gated" if arm == "gated" else (arm == "learned")
            row = run_arm(seed, m, n_cycles, learn_anchors=la, edition=edition)
            rows.append(row)
...
def special_sweep(label: str, m: int, n_cycles: int, edition: int = 2) -> None:
    ...
    tag = label if (edition == 1 or "gated" in label) else f"{label}-v{edition}"
...
def main() -> int:
    ...
    if phase == "selftest":
        selftest()
        return 0
    if phase in ("frozen", "learned", "gated"):
