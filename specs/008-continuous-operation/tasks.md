...
- [ ] T001 `Config.episode_mode` (`"episodic"` default / `"continuous"`) +
      validation in `src/pra/config.py` (data-model table; FR-009)

## Phase 2: Foundational

- [ ] T002 `SingleBootWorld` test instrument + unit tests in
...
- [ ] T003 [US1] Engine continuous branch in `src/pra/core/engine.py`:
...
- [ ] T004 [US1] Integration `tests/integration/test_continuous.py`:
...
- [ ] T005 [US2] Boundary-placement tests (SC-004): chain break, fair-judge
...
- [ ] T006 [US2] World-state capture protocol: `state_dict`/
...
- [ ] T007 [US2] Snapshot support: optional `SystemState.world_state`
...
- [ ] T008 [US2] Composition: continuous + drives, continuous + Body,
      continuous + ladder world — deterministic full runs, no special cases
      (research R7)

## Phase 5: US3 — the reading (P2)

- [ ] T009 [US3] Run the investigatory episodic-vs-continuous reading
