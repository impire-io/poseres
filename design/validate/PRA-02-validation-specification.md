...
*(Scaled measurement, 2026-07-12, `pra-validate scale --t3`, 2000-cycle protocol: effort clause 8/8 at every scale; identity clause **FAILs at `true_dim` 20 and 35** (2/8, margins −0.054 ± 0.075 / −0.030 ± 0.039) and passes at 50 by the thinnest majority (5/8, −0.005 ± 0.036). Diagnosed: substantially measurement composition — the scaled ecology's standing juvenile conveyor sits inside the per-step elect mean, the no-consolidation ablation arms have no churn; churn-matched, predictive beats persistence 6/6 (td 20 and 35, seeds 1–3) with ~⅓ the reference margin. T3 as written is not scale-portable; verdicts recorded as measured, successor (a churn-matched scaled form) named openly. Full trail: `T3SCALE-DIAGNOSIS.md`.)*
