...
budget-bound). Open research, in priority order: the curiosity/competence
blend for non-uniformly-learnable worlds (the A3/A4 ladder), predicted-LP
lookahead, the elect-gating question (why composition pollution shrinks
as the conveyor grows), snapshot support for anatomy-resized runs, and
(design-level, when a deployment demands it) whether reference-scale long
lifetimes eventually need the cap too.
