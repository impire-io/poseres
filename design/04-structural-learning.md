...
**Scale rule (resolved — THRESHOLD-DIAGNOSIS, 2026-07-11): the conveyor correction.** The 5.3 divisor counts the standing conveyor of youth-protected juveniles (`spawn_per_cycle × min_age_cycles` at the effective patience) — frames that cannot be evicted tighten the bar for the frames that can, and at scale the mature niche empties (PROPOSAL-DIAGNOSIS census). The fix is a **coupled pair**: the fair judge (`score_window_steps`, Doc 03 §4) plus the constant-free conditional effective form `effective_survive_threshold_pop_baseline = survive_threshold_pop_baseline + spawn_per_cycle·(effective_min_age_cycles − min_age_cycles)` (PRA-01 §8.8). Measured at `true_dim` 20/35/50: 8/8 seeds anchored with no residual base factor; a fitted base-exponent form was tried first and under-extrapolated at the third scale. The correction **alone** (all-step judge) is measured to land the ecology at the tracking-flattered surface's minimum (anchors 4–7 vs the fair judge's 6–11), hence the conditionality. Verify with the population census instrument (Doc 06 snapshot of per-frame dim/age/score). The once-named successor "fair inter-age comparison" dissolved under measurement (INTERAGE-DIAGNOSIS): time-to-asymptote is flat in dim and `best_dim` churns freely among mature frames — the landing level is set by the flatness of the score surface's basin, i.e. the Doc 03 §6 parsimony-vs-marginal-information question, not by any age asymmetry.
