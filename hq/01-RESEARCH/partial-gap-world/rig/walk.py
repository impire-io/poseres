...
def main() -> int:
    for rung in sys.argv[1:]:
        subprocess.run([PY, str(RIG / "pgap.py"), "solo", rung], check=True)
        m = metrics(f"{rung}-solo")
        print(f"P0 {rung} {json.dumps(m)}", flush=True)
        if not (m["starv"] == 0 and m["eats"] >= 3 and m["below12_ss"] <= 0.10):
            print(f"P0 FAIL {rung} — rung disqualified, no hostile arm", flush=True)
            continue
        subprocess.run([PY, str(RIG / "pgap.py"), "hostile", rung], check=True)
        h = metrics(f"{rung}-hostile")
        print(f"P1 {rung} {json.dumps(h)}", flush=True)
