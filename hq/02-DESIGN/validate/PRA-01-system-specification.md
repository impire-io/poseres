...
**Learned channel weighting (opt-in; CHANNELWEIGHT-DIAGNOSIS, feature 016).** With `channel_weight_floor = f > 0`, both norms take the weighted form `||(·)⊙w|| / (||obs⊙w|| + 1e-6)` with one store-level per-channel weight vector `w ∈ [f, 1]` (numerator and denominator — the ratio stays "relative error over the channels that count"). At `f = 0` (the default) the expressions above execute verbatim, byte-identically.
...
**Learned channel weighting (`channel_weight_floor` / `channel_stats_decay`, CHANNELWEIGHT-DIAGNOSIS 2026-07-18).** Opt-in; the default (0) is byte-identical. When on, the store maintains per-channel EMAs of the raw observation stream (mean, variance, lag-1 covariance; decay β), recomputes weights `w = clip(ρ̂ / max ρ̂, floor, 1)` **only at episode starts** (readiness `ceil(1/(1−β))` samples; unready channels stay at 1), and passes `w` into every frame operation: the weighted survival norms (Section 5.2), the weighted encoder input and placement error (Section 5.5's encoder gradient uses the weighted input the forward pass saw), transitions inheriting through their encodes. The survival EMAs accumulate the weighted norms; telemetry keeps the unweighted definitions. The estimator consumes **no RNG** (paired on/off runs share the world stream up to election-boundary birth shifts — measured tiny), and its state is snapshot state (Doc 06 §2). No new scale rule is needed: estimator convergence is per-step and `obs_dim`-independent while `effective_min_age_cycles` grows at scale, so weights are converged before any judgment at every scale.
