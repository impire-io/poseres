"""
Pose Resolution Architecture (PRA) — testbed v2: zero-start, nonlinear, multi-seed.

WHAT CHANGED FROM v1 AND WHY
----------------------------
1. ZERO-START. The agent begins with NO frames. The first observation that no
   frame can map triggers the birth of a frame built to fit it. After that,
   structure GROWS by the normal spawn-and-select rule. This tests the real PRA
   claim — that structure emerges from nothing — instead of pruning a hand-seeded
   list of dimensionalities. This is the stronger, more honest test.

2. NONLINEAR FRAMES. Each frame's encoder and transition model can BEND (a tanh
   nonlinearity + a hidden layer), not just scale. Reason: the toy world is
   nonlinear. A purely linear frame has a hard representational ceiling unrelated
   to its dimensionality, which would confound the dimensionality result — you
   couldn't tell whether selection failed or whether "flat paper can't wrap a
   ball." Nonlinearity removes that confound.

3. MULTI-SEED. Every configuration runs across several random seeds and we report
   the SPREAD, not a single number. A single pretty result can be luck. This is
   non-negotiable for trusting any claim about whether selection finds structure.

4. CANDIDATE PROPOSALS ARE BIASED toward the current best frame's dimensionality
   (plus occasional exploration), instead of a blind +/-1 random walk. This lets
   selection cover a larger dimensionality range without relying on luck.

5. POPULATION-SCALED DECAY. Removal pressure rises as the population grows, so
   "earned persistence" doesn't silently break when many frames accumulate.

WHAT IS STILL DELIBERATELY SIMPLE
---------------------------------
   * The world is a small synthetic sensorimotor world (hidden latent space,
     lawful action-driven motion, linear+nonlinear emission). It is a TEST
     HARNESS, not a claim about real sensory data.
   * No vector DB. Frames are small learnable modules. Storage/retrieval (e.g.
     Chroma) belongs in the deployment phase, underneath this logic, not in it.

THE CLAIMS TESTED (same spirit as v1, now under harder conditions)
------------------------------------------------------------------
[T1] Sparsity by pull        — frames drop observations, they don't all listen.
[T2] Prediction anchors      — mean prediction error falls as the system learns.
[T3] Effort-only fails       — an agent that scores on effort alone does NOT
                               reduce prediction error (the ablation).
[T4] Structure GROWS         — starting from zero frames, the population grows and
                               its best frame's dimensionality lands at/near the
                               true latent dimensionality, ACROSS SEEDS.
[T5] Decay is default        — frames that never predict well are removed.
[T6] No-loss guard           — the fraction of observations no frame maps stays
                               bounded once the system has warmed up.

Run:  python pra_sim_v2.py
"""
    ...
    def online_episode(self, steps=40):
        ...
        for _ in range(steps):
            ...
            for f in self.frames:
                if f.fit_quality(obs) < self.FIT_GATE:    # frame ELECTS to map
                    mapped += 1
...
def main():
    ...
    print("=" * 74)
    print("PRA TESTBED v2 — zero-start, nonlinear, %d seeds (true latent dim = %d)"
          % (len(seeds), TRUE_DIM))
    print("=" * 74)
